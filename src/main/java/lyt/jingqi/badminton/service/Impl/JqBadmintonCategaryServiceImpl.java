...
import lyt.jingqi.badminton.dao.GoodsCategoryMapper;
...
@Service
public class JqBadmintonCategaryServiceImpl implements JqBadmintonCategoryService {
    ...
    public String updateGoodsCategory(GoodsCategory goodsCategory) {
        ...
        GoodsCategory temp2 = goodsCategoryMapper.selectByLevelAndName(goodsCategory.getCategoryLevel(),
                goodsCategory.getCategoryName());
    ...
    @Override
    public List<JqBadmintonIndexCategoryVO> getCategoriesForIndex() {
        List<JqBadmintonIndexCategoryVO> jqBadmintonIndexCategoryVOS = new ArrayList<>();
        //获取一级分类的固定数量的数据
        List<GoodsCategory> firstLevelCategories =
                goodsCategoryMapper.selectByLevelAndParentIdsAndNumber(Collections.singletonList(0L),
                        JqBadmintonCategoryLevelEnum.LEVEL_ONE.getLevel(), Constants.INDEX_CATEGORY_NUMBER);
        if (!CollectionUtils.isEmpty(firstLevelCategories)) {
            ...
            List<GoodsCategory> secondLevelCategories =
                    goodsCategoryMapper.selectByLevelAndParentIdsAndNumber(firstLevelCategoryIds,
                            JqBadmintonCategoryLevelEnum.LEVEL_TWO.getLevel(), 0);
            if (!CollectionUtils.isEmpty(secondLevelCategories)) {
                ...
                List<GoodsCategory> thirdLevelCategories =
                        goodsCategoryMapper.selectByLevelAndParentIdsAndNumber(secondLevelCategoryIds,
                                JqBadmintonCategoryLevelEnum.LEVEL_THREE.getLevel(), 0);
                if (!CollectionUtils.isEmpty(thirdLevelCategories)) {
                    ...
                    for (GoodsCategory secondLevelCategory : secondLevelCategories) {
                        ...
                        if (thirdLevelCategoryMap.containsKey(secondLevelCategory.getCategoryId())) {
                            ...
                            List<GoodsCategory> tempGoodsCategories =
                                    thirdLevelCategoryMap.get(secondLevelCategory.getCategoryId());
                            secondLevelCategoryVO.setThirdLevelCategoryVOS((BeanUtil.copyList(tempGoodsCategories,
                                    ThirdLevelCategoryVO.class)));
                    ...
                    if (!CollectionUtils.isEmpty(secondLevelCategoryVOS)) {
                        ...
                        for (GoodsCategory firstCategory : firstLevelCategories) {
                            JqBadmintonIndexCategoryVO jqBadmintonIndexCategoryVO = new JqBadmintonIndexCategoryVO();
                            BeanUtil.copyProperties(firstCategory, jqBadmintonIndexCategoryVO);
                            //如果该一级分类下有数据则放入 jqBadmintonIndexCategoryVOS 对象中
                            if (secondLevelCategoryVOMap.containsKey(firstCategory.getCategoryId())) {
                                ...
                                jqBadmintonIndexCategoryVO.setSecondLevelCategoryVOS(tempGoodsCategories);
                                jqBadmintonIndexCategoryVOS.add(jqBadmintonIndexCategoryVO);
                            }
                        }
                    }
                }
            }
            return jqBadmintonIndexCategoryVOS;
        } else {
            return null;
        }
    }
    ...
    public SearchPageCategoryVO getCategoriesForSearch(Long categoryId) {
        ...
        if (thirdLevelGoodsCategory != null
                && thirdLevelGoodsCategory.getCategoryLevel() == JqBadmintonCategoryLevelEnum.LEVEL_THREE.getLevel()) {
            //获取当前三级分类的二级分类
            GoodsCategory secondLevelGoodsCategory =
                    goodsCategoryMapper.selectByPrimaryKey(thirdLevelGoodsCategory.getParentId());
            if (secondLevelGoodsCategory != null && secondLevelGoodsCategory.getCategoryLevel() ==
